...
if __name__ == "__main__":
    ...
    p.add_argument("-o", type=argparse.FileType('w'),
                   default=sys.stdout, help="Outfile. Defaults to stdout.")
    args = p.parse_args()
    centers = [np.random.random((args.d,)), np.random.random((args.d,))]
    while np.linalg.norm(centers[0] - centers[1]) < .5:
        centers = [np.random.random((args.d,)), np.random.random((args.d,))]
    datafirsthalf = [np.random.random((args.d,)) / 10 +
                     centers[0] for i in range(args.n >> 1)]
    datasecondhalf = [np.random.random((args.d,)) / 10 +
                      centers[1] for i in range(args.n - len(datafirsthalf))]
